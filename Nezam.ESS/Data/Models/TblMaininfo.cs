namespace Nezam.ESS.backend.Data.Models;

public class TblMaininfo
{
    public int Id { get; set; }

    public string? SabtDate { get; set; }

    public string? Name { get; set; }

    public string? Fname { get; set; }

    public string? DastoorNo { get; set; }

    public string? DastoorDate { get; set; }

    public string? PelakNo { get; set; }

    public double? Metraj { get; set; }

    public double? MetrajGabl { get; set; }

    public double? TafavotMetraj { get; set; }

    public int? TabageNo { get; set; }

    public int? TedadSagf { get; set; }

    public bool? CalcMojri { get; set; }

    public string? DaftarNo { get; set; }

    public int? Gorooh { get; set; }

    public int? Mantageh { get; set; }

    public string? ParvaneNo { get; set; }

    public string? ParvanehDate { get; set; }

    public int? ParvanehMetraj { get; set; }

    public int? Ok { get; set; }

    public string? Comment { get; set; }

    public int? ChangeType { get; set; }

    public int Sal { get; set; }

    public int? Mah { get; set; }

    public int? Rooz { get; set; }

    public long Radif { get; set; }

    public string? SodoorDate { get; set; }

    public string? SodoorTime { get; set; }

    public string? SabtTime { get; set; }

    public string? MalekAddress { get; set; }

    public string? MalekMobNo { get; set; }

    public long? MapNezaratMablag { get; set; }

    public long? MojriCod { get; set; }

    public int? RdGeymati { get; set; }

    public int? KarbarId { get; set; }

    public int? SkeletType { get; set; }

    public int? KarbariType { get; set; }

    public double? TakhfifPerc { get; set; }

    public int? MahalEhdasType { get; set; }

    public double? TarakomPerc { get; set; }

    public int? MalekiyatType { get; set; }

    public int? SahmiehType { get; set; }

    public string? MaliComment { get; set; }

    public int? MaliTayidType { get; set; }

    public int? MaliTayidKarbarId { get; set; }

    public DateTime? MaliDtayid { get; set; }

    public string? MalekMelliCod { get; set; }

    public string? MalekAddressEgamat { get; set; }

    public string? MalekVakilName { get; set; }

    public int? RdGeymatiTaarefe { get; set; }

    public string? ExtraComments { get; set; }

    public bool? TaahodMojri { get; set; }

    public string? NosaziCod { get; set; }

    public int? DNemayandegiCod { get; set; }
}