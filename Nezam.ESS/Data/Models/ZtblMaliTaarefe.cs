namespace Nezam.ESS.backend.Data.Models;

public class ZtblMaliTaarefe
{
    public int Id { get; set; }

    public int? Sal { get; set; }

    public int? RdGeymati { get; set; }

    /// <summary>
    ///     nezarat or tarrahi
    /// </summary>
    public int? CodKhadamatCat { get; set; }

    public int? CodReshteh { get; set; }

    public long? F1 { get; set; }

    public long? F2 { get; set; }

    public long? F3 { get; set; }

    public long? F4 { get; set; }

    public long? F5 { get; set; }

    public long? F6 { get; set; }

    public long? F7 { get; set; }

    public long? F8 { get; set; }

    public long? F9 { get; set; }

    public long? F10 { get; set; }

    public long? F11 { get; set; }

    public long? F12 { get; set; }

    public long? F13 { get; set; }

    public long? F14 { get; set; }

    public long? F15 { get; set; }

    public long? F16 { get; set; }

    public long? F16plus { get; set; }
}